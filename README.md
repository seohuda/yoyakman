...
**최근 대화 요약**
- `/요약` 입력 → 최근 메시지 **개수**만큼 수집 (기본 50, 최대 300)
- `/요약 개수:20`처럼 원하는 개수를 지정할 수 있어요 (5~300)

**특정 메시지부터 요약**
- 요약을 시작할 메시지 **우클릭 → 앱 → 이 메시지부터 요약** (해당 메시지부터 최대 300개 수집)

슬래시 커맨드의 **개수** 옵션으로 `/요약` 수집량을 조절한다. 컨텍스트 메뉴는
`bot.py`의 `MAX_MESSAGES`(300)까지 읽는다. 디스코드 API는 한 요청당 100개가
상한이지만 discord.py가 100개씩 나눠 받아오므로, 값만 바꾸면 된다(300이면 요청 3회).
모델이 출력 길이에 걸려 요약을 중간에 끊으면 앞부분까지 보내고 안내를 덧붙인다.
