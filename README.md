...
**최근 대화 요약**
- `/요약` 입력 → 최근 메시지 최대 300개 수집 (300개가 안 되면 있는 만큼)

**특정 메시지부터 요약**
- 요약을 시작할 메시지 **우클릭 → 앱 → 이 메시지부터 요약** (해당 메시지부터 최대 300개 수집)

수집 개수는 `bot.py`의 `MAX_MESSAGES`로 조절한다. 디스코드 API는 한 요청당 100개가
상한이지만 discord.py가 100개씩 나눠 받아오므로, 값만 바꾸면 된다(300이면 요청 3회).
대화가 길어 요약이 출력 길이(`MAX_OUTPUT_TOKENS`)에 걸리면 잘린 앞부분까지 보내고
끊겼다는 안내를 덧붙인다.
