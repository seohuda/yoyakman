...
**최근 대화 요약**
- `/요약` → 채널 **맨 아래(최근)** 대화를 요약 (있는 만큼, 최대 300)

**특정 메시지부터 요약**
- 요약 시작할 메시지 **우클릭 → 앱 → 이 메시지부터 요약**
- 선택한 메시지 **아래쪽(이후)** 대화만 있는 만큼 모은다. 채널 최근 300개와 다르다.

`/요약`과 **이 메시지부터 요약**은 다릅니다. 메시지 밑 대화만 보려면 우클릭 메뉴를 쓰세요.

디스코드 API는 한 요청당 100개가 상한이지만 discord.py가 100개씩 나눠 받아온다(300이면 요청 3회).
